...
c_b_bool=$(yq e '.classification.localBlastBool' "$env_config_path")
c_b_maxaccepts=$(yq e '.classification.localBlast.maxaccepts' "$env_config_path")
c_b_perc_identity=$(yq e '.classification.localBlast.percIdentity' "$env_config_path")
c_b_query_cov=$(yq e '.classification.localBlast.queryCov' "$env_config_path")
c_v_bool=$(yq e '.classification.localVsearchBool' "$env_config_path")
...
qiime tools import --type 'SampleData[PairedEndSequencesWithQuality]' --input-path output/metadata/manifest.csv --output-path analysis/s01_16S_eDNA_Demux.qza  --input-format PairedEndFastqManifestPhred33
...
qiime cutadapt trim-paired --i-demultiplexed-sequences analysis/s01_16S_eDNA_Demux.qza --p-front-f ACGAGAAGACCCYRTGGARCTT --p-front-r ATCCAACATCGAGGTCGTAA --p-error-rate $c_error_rate --p-match-adapter-wildcards True --p-match-read-wildcards True --p-discard-untrimmed True --p-times 10 --o-trimmed-sequences analysis/s03_lead_primertrimmed_16S_eDNA_Demux.qza

# s04
# Trim primers from paired-end demux (lag)
qiime cutadapt trim-paired --i-demultiplexed-sequences analysis/s03_lead_primertrimmed_16S_eDNA_Demux.qza --p-adapter-f TTACGACCTCGATGTTGGAT --p-adapter-r AAGYTCCAYRGGGTCTTCTCGT --p-error-rate 0.3 --p-match-adapter-wildcards True  --p-match-read-wildcards True  --p-discard-untrimmed False  --p-times 10  --o-trimmed-sequences analysis/s04_leadlag_primertrimmed_16S_eDNA_Demux.qza

# s05
# Summarize into a qzv file for viewing on QIIME2View
echo "Summarizing qzv for viewing of trimmed primers on QIIME2View"
qiime demux summarize  --i-data analysis/s04_leadlag_primertrimmed_16S_eDNA_Demux.qza  --o-visualization analysis/s05_primertrimmed_16S_eDNA_Demux.qzv 

# s06
# Denoise paired sequences with dada2
echo "Denoising sequences with dada2"
qiime dada2 denoise-paired --i-demultiplexed-seqs analysis/s04_leadlag_primertrimmed_16S_eDNA_Demux.qza --p-trunc-q $c_trunc_q --p-trunc-len-f $c_trunc_len --p-trunc-len-r $c_trunc_len --p-n-threads 0 --output-dir analysis/s06_denoised_16S_eDNA

qiime tools extract --input-path analysis/s06_denoised_16S_eDNA/table.qza --output-path analysis/s06_denoised_16S_eDNA/sample_table

biom convert -i analysis/s06_denoised_16S_eDNA/sample_table/*/data/feature-table.biom -o analysis/s06_denoised_16S_eDNA/sample_table/feature-table.tsv --to-tsv
...
# if a classifier dir is provided
