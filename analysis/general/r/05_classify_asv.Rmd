...
# output file
classification_out = here(run_dir, "output", paste0(config$run$name, "_vsearch_hybrid_classification.csv"))
...
```{r}
# read in hits
hits_raw = read_csv(here(run_dir, "output", paste0(config$run$name, "_eDNA_hits_joined.csv")))

# read in taxa + geography
taxa_geography = read.csv(here(run_dir, "output", paste0(config$run$name, "_taxonomy_table_gbif_occ.csv")))
  ...
  left_join(taxa_geography %>%
              select(taxon_id,
                     any_of(c("specificEpithet",
                              ...
                              "gbif_count_species_local"))), by = "taxon_id") %>%
...
hits_accept = hits_geography %>%
  ...
  select("asv_id", "asv_total_count", "sample_count", "samples", "sequence", 
         "method", "taxon_id", "domain", "kingdom", "phylum", "class", 
...
write.csv(hits_accept, classification_out, row.names = FALSE)
